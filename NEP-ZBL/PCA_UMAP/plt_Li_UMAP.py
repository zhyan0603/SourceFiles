...
data_200 = np.load('des_Li_802.npy')   
data_25 = np.load('des_Li_25.npy')      

combined_data = np.vstack((data_1978, data_200, data_25))  

labels_1978 = np.zeros(data_1978.shape[0]) 
labels_200 = np.ones(data_200.shape[0])        
labels_25 = np.full(data_25.shape[0], 2)    

combined_labels = np.hstack((labels_1978, labels_200, labels_25))  

umap_model = umap.UMAP(n_components=2, random_state=42)  
reduced_data = umap_model.fit_transform(combined_data)  
...
ax_scatter.set_xlabel('UMAP Component 1')
ax_scatter.set_ylabel('UMAP Component 2')
ax_scatter.legend()
...
#plt.show()
plt.savefig('Figure5_UMAP.png', dpi=600)