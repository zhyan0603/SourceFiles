...
fig, axs = plt.subplots(1, 3, figsize=(11, 3.1), dpi=600)


# Plotting the train_data figure
axs[0].plot(train_data[:, 1], train_data[:, 0], '.', markersize=10)
axs[0].plot(np.arange(-7.48, -7.12, 0.01), np.arange(-7.48, -7.12, 0.01), linewidth=2, color='grey', linestyle='--')
...
axs[1].plot(force_test[:, 3:6], force_test[:, 0:3], '.', markersize=10)
axs[1].plot(np.arange(-11.5, 11.5, 0.01), np.arange(-11.5, 11.5, 0.01), linewidth=2, color='grey', linestyle='--')
axs[1].set_xlabel('DFT force (eV/$\AA$)', fontsize=11)
axs[1].set_ylabel('NEP force (eV/$\AA$)', fontsize=11)
...
axs[1].text(0.35, 0.08, f'RMSE: {mean_force_rmse:.2f} meV/$\AA$', transform=axs[1].transAxes, fontsize=11, verticalalignment='center')
axs[1].text(-0.1, 1.03, "(b)", transform=axs[1].transAxes, fontsize=13, va='top', ha='right')

# Plotting the virial figure
axs[2].plot(stress_test[:, 6:12], stress_test[:, 0:6], '.', markersize=10)
axs[2].plot(np.arange(-17, 37, 0.01), np.arange(-17, 37, 0.01), linewidth=2, color='grey', linestyle='--')
